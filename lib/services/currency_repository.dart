...
class CurrencyRepository {
  ...
  static final List<Currency> currencies = [
    Currency(
        symbol: "symbol", code: "code", name: 'name', icon: 'icon', rate: 1.0),
  ];

  CurrencyRepository(
      {required List<Currency> currencies, required String baseRate})
      : _currencies = currencies,
        _baseRate = baseRate {
    assert(_currencies.isNotEmpty);
    assert(_currencies
        .any((currency) => isEqualIgnoreCase(currency.code, _baseRate)));
  }
  ...
  Currency getByCode(String code) {
    var matches =
        _currencies.where((currency) => isEqualIgnoreCase(currency.code, code));
    if (matches.isEmpty) {
      log.error(
          "Found no currency with code [$code] among ${_currencies.length} currencies");
  ...
  void _update(String currencyCode, double newRate) {
    var matches = _currencies
        .where((currency) => isEqualIgnoreCase(currency.code, currencyCode));
    if (matches.isEmpty) {
